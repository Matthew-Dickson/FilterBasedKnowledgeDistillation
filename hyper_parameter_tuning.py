...
parser.add_argument('--loss-function', help='The loss function to use', type=int, default=DISTILLATION_TYPE)
parser.add_argument('--run-on', help='The data set to run on', default=RUN_ON)
parser.add_argument('--run-multiple', help='Run training multiple time', type=bool, default=RUN_MULTIPLE)
parser.add_argument('--run-times', help='Number of K fold splits', type=int, default=RUN_TIMES)
    ...
    for index, config in enumerate(hyper_parameters):
        ...
        if(args.run_multiple):
           for run in range(0,args.run_times):
                ...
                model_histories['Run{}'.format(run+1)] = history 
