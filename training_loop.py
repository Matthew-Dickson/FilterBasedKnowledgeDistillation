...
def train(train_dataset,
        ...
        for epoch in range(number_of_epochs):
                ...
                if(teacher_model != None):
                    ...
                    if(distillation_type == LossType.FILTER.name):
                        ...
                        loss = loss_function(student_logits = logits,
                                                                  ...
                                                                  features = images,
                                                                  teacher_model = teacher_model)
                    elif(distillation_type == LossType.FEATURE.name):
                        loss = loss_function(student_logits = logits,
                                                                labels = labels,
                                                                features = images,
                                                                teacher_model = teacher_model,
                                                                student_model = student_model)
                    elif(distillation_type == LossType.FEATURE_LAST.name):
                        loss = loss_function(student_logits = logits,
                                                                labels = labels,
                                                                features = images,
                                                                teacher_model = teacher_model,
                                                                student_model = student_model)
                    elif(distillation_type == LossType.FILTER_LAST.name):
                        loss = loss_function(student_logits = logits,
                                                                labels = labels,
                                                                features = images,
                                                                teacher_model = teacher_model,
            ...
            if(patience > 0 and early_stopper.early_stop(validation_loss=avg_validation_loss_per_epoch)):
                break
