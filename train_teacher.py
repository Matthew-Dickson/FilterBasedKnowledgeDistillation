...
from functions.loss.feature_map_knowledge_distillation_loss import FeatureMapKnowledgeDistillationLoss
from functions.loss.feature_map_last_knowledge_distillation_loss import FeatureMapLastKnowledgeDistillationLoss
from functions.loss.filter_last_knowledge_distillation_loss import FilterLastKnowledgeDistillationLoss
...
RUN_ON = "CIFAR10"
DISTILLATION_TYPE = 4
SAVE_TEACHER_PATH = './data/models/Resnet110cifar10.pt'
...
def get_loss_function(loss_type, loss_options):
    ...
    elif(LossType.FEATURE.name == LossType(loss_type).name):
        loss_function = FeatureMapKnowledgeDistillationLoss(options=loss_options)
    elif(LossType.FEATURE_LAST.name == LossType(loss_type).name):
        loss_function = FeatureMapLastKnowledgeDistillationLoss(options=loss_options)
    elif(LossType.FILTER_LAST.name == LossType(loss_type).name):
        loss_function = FilterLastKnowledgeDistillationLoss(options=loss_options)
...
if __name__ == '__main__':
    ...
    if(args.run_on == "CIFAR10"):
        input_channels = 3
        output_channels = 10
        train_val_dataset = CIFAR10(root='data/', train=True, download=True, transform=cifar_training_augmentation())
        test_dataset = CIFAR10(root='data/', train=False,transform=cifar_testing_augmentation())
        TEACHER_MODEL = resnet110()
    ...
    if(args.run_on == "TAR"):
        input_channels = 3
        output_channels = 2
        TEACHER_MODEL = resnet110(num_classes=2) #200x200
    ...
    if(args.run_on == "TAR"):
        ...
        tar_datasets = {x: datasets.ImageFolder(root=f'./data/tar/{x}', transform = data_transforms[x])
                         for x in ['train','test','valid']}
        train_dataset = tar_datasets['train']
        valid_dataset = tar_datasets['valid']
        test_dataset = tar_datasets['test']
        class_names = tar_datasets['train'].classes
